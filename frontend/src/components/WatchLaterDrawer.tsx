...
interface WatchLaterDrawerProps {
  isOpen: boolean;
  onClose: () => void;
  watchLater: Movie[];
...
const WatchLaterDrawer = ({ isOpen, onClose, watchLater, onRemoveFromWatchLater }: WatchLaterDrawerProps) => {
  return (
    <Sheet open={isOpen} onOpenChange={onClose}>
      <SheetContent side="right" className="glass border-l border-border/30 w-full sm:max-w-md">
        <SheetHeader>
          <SheetTitle className="flex items-center gap-2">
            <Clock className="w-5 h-5 text-mood-thoughtful" />
            Sonra Bax
          </SheetTitle>
        </SheetHeader>
          ...
          {watchLater.length === 0 ? (
            <div className="flex flex-col items-center justify-center py-16 text-center">
              ...
              <p className="text-muted-foreground">
                Sonra baxmaq üçün hələ heç nə əlavə etməmisiniz
              </p>
            </div>
          ) : (
            watchLater.map((movie, index) => (
              ...
              >
                ...
                <Button
                  variant="ghost"
                  size="icon"
                  onClick={() => onRemoveFromWatchLater(movie.id)}
